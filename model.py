...
if pse_model_type == "mobilenetv2":
    text_detect_net = PSENet(
        backbone=pse_model_type, pretrained=False, result_num=6, scale=pse_scale
    )
...
if crnn_type == "full_lstm" or crnn_type == "full_dense":
    crnn_net = FullCrnn(
        32, 1, len(alphabet) + 1, nh, n_rnn=2, leakyRelu=False, lstmFlag=LSTMFLAG
    )
...
if crnn_vertical_model_path is not None:
    crnn_vertical_net = LiteCrnn(
        32, 1, len(alphabet) + 1, nh, n_rnn=2, leakyRelu=False, lstmFlag=True
    )
    crnn_vertical_handle = CRNNHandle(
        crnn_vertical_model_path, crnn_vertical_net, gpu_id=GPU_ID
    )
...
def crnnRec(im, rects_re, leftAdjust=False, rightAdjust=False, alph=0.2, f=1.0):
    ...
    for index, rect in enumerate(rects_re):
        ...
        try:
            ...
            if crnn_vertical_handle is not None and angel_class in [
                "shudao",
                "shuzhen",
            ]:

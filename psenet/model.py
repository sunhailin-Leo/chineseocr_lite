...
d = {
    "resnet18": {"model": resnet18, "out": [64, 128, 256, 512]},
...
class PSENet(nn.Module):
    def __init__(self, backbone, result_num=6, scale: int = 1, pretrained=False):
        ...
        self.smooth1 = nn.Sequential(
            nn.Conv2d(
                conv_out, conv_out, kernel_size=3, stride=1, padding=1, groups=conv_out
            ),
            nn.BatchNorm2d(conv_out),
            nn.ReLU(inplace=inplace),
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.train:
            ...
            x = F.interpolate(
                x, size=(H // self.scale, W // self.scale), mode="nearest"
            )
        return x
